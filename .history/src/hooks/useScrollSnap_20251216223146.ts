...
export function useScrollSnap() {
  ...
  useEffect(() => {
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      // Block if we just handled a scroll (debounce) - very aggressive to prevent double jumps
      ...
      // Get current section based on actual scroll position, not the ref
      const currentScroll = window.scrollY
      ...
      // Use the actual current index, but update the ref
      ...
      if (targetSection) {
        ...
        // Reset flag after scroll animation completes - longer timeout
    ...
    // Also listen for scroll end to reset the flag - but don't let it interfere
    const handleScroll = () => {
      // Only reset if we're not in the middle of a wheel-triggered scroll
      if (scrollEndTimeoutRef.current) {
        clearTimeout(scrollEndTimeoutRef.current)
      }
      scrollEndTimeoutRef.current = window.setTimeout(() => {
        // Only reset if enough time has passed since last wheel event
